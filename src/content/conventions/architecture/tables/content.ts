...
export const content: ContentBlock[] = [
  ...
  {
    type: 'paragraph',
    text: "A maintenance or pipeline read — a backlog count, a status check, a \"what's the next batch\" fetch, a run-id allocator — always opts out of the cache explicitly. That kind of read's entire purpose is finding out what the live database looks like right now; served from a stale cache entry, it can silently repeat the same answer indefinitely, which is a much worse failure than the same staleness would be on an ordinary display page.",
  },
]
